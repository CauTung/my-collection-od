 ...
 * app/routes/home.tsx
 *
 * Root route — serves two purposes:
 * 1. App Proxy entry point: When Shopify App Proxy forwards a request here,
 *    it includes signature/shop/logged_in_customer_id params.
 *    We verify HMAC and return a Liquid-compatible HTML fragment.
 * 2. Direct access: Shows a simple info page when accessed directly.
 *
 * CRITICAL: Shopify App Proxy forwards requests to the ROOT of the proxy URL.
 * When a user visits https://shop.myshopify.com/apps/my-collection,
 * Shopify forwards to https://our-backend.com/ (root path).
 * That's why the App Proxy handler MUST be on the index route.
 */
...
<style>
  ...
  .dc-login-prompt h2 { margin-bottom: 1rem; color: #1a1a2e; }
...
<div class="dc-app" id="dc-app">
  ${!customerId ? `
    <div class="dc-login-prompt">
      ...
      <a href="/account/login">Log In to Your Account &rarr;</a>
    </div>
  ` : `
...
(function() {
  ...
  function renderStats(stats) {
    ...
    container.innerHTML =
      ...
      '<div class="dc-stat"><div class="dc-stat-label">Collection Value</div><div class="dc-stat-value">$' + Number(stats.total_value || 0).toLocaleString() + '</div></div>';
  }
    ...
    container.innerHTML = items.map(function(item) {
      ...
      return '<div class="dc-card">' +
        '<div class="dc-card-img">' +
          (item.image_url
            ? '<img src="' + item.image_url + '" alt="' + (item.product_title || "Coin").replace(/"/g, "&quot;") + '" style="max-height:100%;max-width:100%;object-fit:contain;" />'
        ...
        '<div class="dc-card-body">' +
          ...
          '<div class="dc-card-footer">' +
            '<div class="dc-card-price">' + (item.estimated_value ? "$" + Number(item.estimated_value).toLocaleString() : "&mdash;") + '</div>' +
    ...
    apiFetch("")
      ...
      .catch(function(err) {
        ...
        document.getElementById("dc-loading").innerHTML =
          '<p style="color:#ef476f;">Failed to load collection. Please try refreshing the page.</p>';
      });
  }
  ...
  window.dcSyncPurchases = function() {
    ...
    apiFetch("/sync", { method: "POST" })
      ...
      .finally(function() { btn.disabled = false; btn.innerHTML = "&#x1F504; Sync Past Orders"; });
  };
  ...
  window.dcShowAddForm = function() {
    showToast("Manual add coming soon!");
  };
